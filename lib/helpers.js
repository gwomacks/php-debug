...
exports.generatePathMaps = function(remoteUri, existingPathMaps,aggregate) {
  ...
  for (let mapping of existingPathMaps) {
    if (mapping.remotePath == uriParent) {
  ...
  for (let project of projectDirs) {
    if (project.realPath != undefined && project.realPath != null) {
      ...
      const files = fastGlob.sync(normal);
  ...
  if (aggregate == undefined || aggregate == null) {
    if (rankedListing.length == 0) {
      rankedListing.push({remotePath:uri,localPath:""})
    }
...
exports.localPathToRemote = function(localPath, pathMap) {
...
exports.remotePathToLocal = function(remotePath, pathMap) {
  if (pathMap == undefined || pathMap == null || !pathMap.hasOwnProperty('localPath') || !pathMap.hasOwnProperty('remotePath')) {
