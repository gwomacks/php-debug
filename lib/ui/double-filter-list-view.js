...
export default class DoubleFilterListView {
  ...
  constructor (props) {
    ...
    if (props.hasOwnProperty('firstQuery') && props.firstQuery != null && props.firstQuery != "") {
      this.refs.firstQueryEditor.setText(props.firstQuery)
    }

    if (props.hasOwnProperty('secondQuery') && props.secondQuery != null && props.secondQuery != "") {
      this.refs.secondQueryEditor.setText(props.secondQuery)
    }
    this.disposables.add(this.refs.firstQueryEditor.onDidChange(this.didChangeQuery.bind(this)))
    this.disposables.add(this.refs.secondQueryEditor.onDidChange(this.didChangeQuery.bind(this)))
    if (!props.skipCommandsRegistration) {
      this.disposables.add(this.registerAtomCommands())
    }
    const firstEditor  = this.refs.firstQueryEditor.element
    const secondEditor = this.refs.secondQueryEditor.element
    const didLoseFocus = this.didLoseFocus.bind(this)
    //firstEditor.addEventListener('blur', didLoseFocus)
    //secondEditor.addEventListener('blur', didLoseFocus)
    //this.disposables.add(new Disposable(() => { firstEditor.removeEventListener('blur', didLoseFocus) }))
    //this.disposables.add(new Disposable(() => { secondEditor.removeEventListener('blur', didLoseFocus) }))
    // Should be fixed in https://github.com/atom/atom/pull/16492
    this.refs.firstQueryEditor.component.refs.cursorsAndInput.refs.hiddenInput.setAttribute("tabindex","1")
    this.refs.secondQueryEditor.component.refs.cursorsAndInput.refs.hiddenInput.setAttribute("tabindex","2")
  ...
  focus () {
    this.refs.firstQueryEditor.element.focus()
  }
  ...
  reset () {
    this.refs.firstQueryEditor.setText('')
    this.refs.secondQueryEditor.setText('')
  }
  ...
  update (props = {}) {
      ...
      if (props.hasOwnProperty('firstQuery')) {
      ...
      if (props.hasOwnProperty('secondQuery')) {
        // Items will be recomputed as part of the change event handler, so we
        // don't need to recompute them again at the end of this function.
        this.refs.secondQueryEditor.setText(props.secondQuery)
        shouldComputeItems = false
      ...
      if (props.hasOwnProperty('selectFirstQuery')) {
        if (props.selectFirstQuery) {
          this.refs.firstQueryEditor.selectAll()
        } else {
          this.refs.secondQueryEditor.clearSelections()
        }
      }

      if (props.hasOwnProperty('selectSecondQuery')) {
        if (props.selectSecondQuery) {
          this.refs.secondQueryEditor.selectAll()
        } else {
          this.refs.secondQueryEditor.clearSelections()
        }
      }
    ...
    render () {
     return $.div(
       {},
       this.renderFirstLabel(),
       $(TextEditor, {ref: 'firstQueryEditor', mini: true}),
       this.renderSecondLabel(),
       $(TextEditor, {ref: 'secondQueryEditor', mini: true}),
   ...
   renderFirstLabel () {
     if (this.props.hasOwnProperty("firstLabel")) {
       return $.span({ref: 'firstLabel'}, this.props.firstLabel)
     } else {
       return ''
     }
   }
   ...
   renderSecondLabel () {
     if (this.props.hasOwnProperty("secondLabel")) {
       return $.span({ref: 'secondLabel'}, this.props.secondLabel)
     } else {
       return ''
     }
   }
  ...
  getQuery () {
    if (this.refs && this.refs.firstQueryEditor) {
      return {first:this.refs.firstQueryEditor.getText(), second:this.refs.secondQueryEditor.getText()}
    } else {
      return {first:'',second:''}
    }
  }
   ...
   didChangeQuery () {
     this.initialSelectionChanged = true
   ...
   fuzzyFilter (items, query) {
    if (query.first.length === 0 && query.second.length === 0) {
      return items
    } else {
      const scoredItems = []
      for (const item of items) {
        const filterKeys = this.props.filterKeysForItem ? this.props.filterKeysForItem(item) : item
        const firstString = filterKeys.first
        const secondString = filterKeys.second
        let firstScore = 0
        let secondScore = 0
        if (query.first.length > 0) {
          firstScore = fuzzaldrin.score(firstString, query.first)
        }
        if (query.second.length > 0) {
          secondScore = fuzzaldrin.score(secondString, query.second)
        }
        let score = firstScore + secondScore
        if (score > 0) {
          if (firstString.toLowerCase().includes(query.first.toLowerCase()) && secondString.toLowerCase().includes(query.second.toLowerCase())) {
