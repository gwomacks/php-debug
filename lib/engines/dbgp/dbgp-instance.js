...
import {escapeValue, localPathToRemote, remotePathToLocal, createPreventableEvent} from '../../helpers'
...
export default class DbgpInstance {
  ...
  parse () {
    ...
    while (this._bufferReadIdx < this._bufferIdx) {
      ...
      if ((buffer.length - idx) < (messageLength - writeIdx)) {
        ...
        writeItx = writeIdx + (buffer.length - idx);
        buffer = null;
        _currentBufferReadIdx = 0;
        this._bufferReadIdx++;
        continue;
      }
  ...
  handleData (data) {
    ...
    if (atom.config.get("php-debug.server.protocolDebugging")) {
      this._emitter.emit('php-debug.engine.internal.debugDBGPMessage',{context:this.getContext(),message:message,type:"raw-recieved"})
    }
  ...
  executeBreakpoint (breakpoint) {
    ...
    switch (breakpoint.getSettingValue("type")) {
      ...
      default:
        ...
        path = localPathToRemote(path, this._pathMap)
        commandOptions = {
          t: 'line',
          f: encodeURI(path),
      ...
      (data) => {
        ...
        switch (response.$.status) {
          case 'break':
            ...
            //console.dir data
            let filepath = remotePathToLocal(decodeURI(messageData['filename']), this._pathMap)
